    ...
    SelectList.prototype={
        ...
        renderSelect: function(element){
            var that = this,
                ...
                selectLength = that.getOptionCount($this),
				selectEncryption=$("#sel_x").find("option:selected").attr('data-encryption'),
				selectEnctype=$("#sel_x").find("option:selected").attr('data-enctype'),
				selectChannel=$("#sel_x").find("option:selected").attr('data-channel'),
				
                selectHTML = '<div id="' + selectID + '" class="select-wrapper ' + selectClassName + '"><input type="hidden" name="' + selectName + '" value="' + selectedValue + '" /><input type="hidden" name="encryption" value="' + selectEncryption + '" /><input type="hidden" name="enctype" value="' + selectEnctype + '" /><input type="hidden" name="channel" value="' + selectChannel + '" /><i class="icon select-down"></i><input type="button" class="select-button" value="' + selectedOptionText + '" /><div class="select-list"><ul>',
                selectListHTML = '';
				
            for(var i=0; i<selectLength; i++){
				var item = $('#sel_x').children('option')[i];
				
				var itemencryption =data.split(",")[i];
                if(i !== selectedIndex){
                    selectListHTML = selectListHTML + '<li class="'+item.className+'" data-encryption="' + itemencryption + '" data-value="' + selectOptionValue[i] + '"><span>' + selectOptionText[i] + '</span></li>';
                }else{
                    selectListHTML = selectListHTML + '<li class="'+item.className+'" data-encryption="' + itemencryption + '" data-value="' + selectOptionValue[i] + '" class="selected"><span>' + selectOptionText[i] + '</span></li>';
                }
            ...
            if(that.settings.enable){
                ...
                selectItem.on('click',function(event){
                    ...
                    $(this)
                        ...
                        .prev().val($(this).text())
                        .siblings('input[name="sel_x"]').val($(this).attr('data-value'))
						.siblings('input[name="encryption"]').val($(this).attr('data-encryption'));
